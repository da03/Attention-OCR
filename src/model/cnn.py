...
def var(name, shape, initializer, regularizable):
...
def var_random(name, shape, regularizable=False):
    return var(name, shape, tf.contrib.layers.xavier_initializer(), regularizable)

def tf_2x2pool(incoming, name):
    with tf.variable_scope(name):
        return tf.nn.max_pool(incoming, ksize=(1, 2, 2, 1), strides=(1, 2, 2, 1), padding='VALID')

def tf_2x1pool(incoming, name):
    with tf.variable_scope(name):
...
def tf_bn(incoming, is_training):
...
def ConvReluBN(incoming, num_filters, filter_size, name, is_training, padding_type = 'SAME'):
    ...
    with tf.variable_scope(name):
        ...
        after_bn = tf_bn(after_conv, is_training)
...
class CNN(object):
    ...
    def _build_network(self, input_tensor, is_training):
        ...
        print('CNN outdim: {}'.format(net.get_shape()))  # 1x32x100 -> 24x512
        self.model = net
